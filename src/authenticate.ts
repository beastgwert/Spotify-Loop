...
export async function getAccessToken() {
  const { accessToken, expiresIn } = await chrome.storage.local.get(["accessToken", "expiresIn"]);
...
export const authenticate = async () => {
  ...
  try {
    ...
    const authorizationCode = new URL(redirectUriWithCode).searchParams.get("code");
