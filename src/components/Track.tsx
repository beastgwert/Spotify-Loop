...
const Track: React.FC<TrackProps> = ({queue, queueIndex, setQueueIndex, setQueue, changePlaying, ind, trackInfo}) => {
    ...
    return <Box sx={{display: 'flex', gap: '0.5rem', height: '3.5rem', pt: '0.5rem', padding: '0.5rem', '&:hover': {opacity: 0.8, bgcolor: 'primary.hover'}}}
        ...
        await setStorageData({nextTrack: !nextTrack || nextTrack == -1 ? 1 : -1});
        }}/>
        <Box className = "song-info" sx={{flex: 1, display: 'flex', alignItems: 'center', '& .MuiTypography-root': {fontSize: '0.6rem', cursor: 'default'}}}>
            ...
            <Box sx={{flex: '1', display: 'flex', justifyContent: 'space-between'}}>
                <Typography color="primary.contrastText" sx={{maxWidth: '80%'}}>{trackInfo.album.name}</Typography>
